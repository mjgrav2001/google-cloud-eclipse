  ...
  public void detect(IFacetedProjectWorkingCopy fpjwc, IProgressMonitor monitor)
    ...
    try (InputStream content = appEngineWebXml.getContents()) {
      ...
      if (descriptor.getRuntime() == null || !"java7".equals(descriptor.getRuntime())) {
        logger.fine("skipping " + fpjwc.getProjectName() + ": appengine-web.xml is not java7");
        return;
      }
      ...
      if (!fpjwc.hasProjectFacet(JavaFacet.FACET)) {
        logger.fine(fpjwc.getProjectName() + ": setting Java 7 facet");
        fpjwc.addProjectFacet(JavaFacet.VERSION_1_7);
      }
      if (!fpjwc.hasProjectFacet(WebFacetUtils.WEB_FACET)) {
        logger.fine(fpjwc.getProjectName() + ": setting Dynamic Web 2.5 facet");
        fpjwc.addProjectFacet(WebFacetUtils.WEB_25);
      }
